#Marcelo Henrique Araújo Garcia
...
#acessando vetores
#acessando os elementos (que está na linha 84 - vetor)
#acessa os vetores por meio de []
codes[2] #segundo elemento (começa a contar do 1)
codes[c(1,3)] #só tem dois elementos o 1 e o 3
...
#Coerção
#o R faz a coerção automatica (faz um possivel ajuste)
x <-c(1, "canada", 3)
class(x)
print(x)
...
e[ ,1:2] #
e[1:2,1] #vai imprimira primeira coluna.
as.data.frame(e) #vai visualizar o "e" como dataframe. é um tipo de conversão.
...
max(murders$total)
i_max <-which.max(murders$total) #vai colocar o indice maximo. valor maximo.
i_max #vai mostrar o quinto Estado que é o que tem o valor maximo.
murders$state[i_max] #vai imprimir "california" que é onde tem mais murder
min(murders$total)
